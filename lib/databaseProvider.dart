...
class DatabaseProvider {
  ...
  static final String paymenttableName = 'payment';
  ...
  Future<List<Payment>> get payment async {
    final db = await database;
    List<Map> result = await db.query(paymenttableName);
    List<Payment> payment = [];
    for (var value in result) {
      payment.add(Payment.fromMap(value));
    }
    return payment;
  }

  Future<List<Note>> get note async {
    final db = await database;
    List<Map> result = await db.query(notetableName);
    List<Note> note = [];
    for (var value in result) {
      note.add(Note.fromMap(value));
    }
    return note;
  }
  ...
  Future insertPayment(Payment payment) async {
    final db = await database;
    return await db.insert(paymenttableName, payment.toMap());
  }

  Future insertNote(Note note) async {
    final db = await database;
    return await db.insert(notetableName, note.toMap());
  }
